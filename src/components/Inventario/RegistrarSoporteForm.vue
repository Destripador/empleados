	...
	<div class="support-form-shell">
		...
		<form v-else @submit.prevent="submitSupport">
			<section aria-labelledby="support-device-heading">
				...
				<div v-if="!selectedDevice && !hideDeviceSearch" class="support-search">
					<NcTextField
						ref="searchField"
						:value.sync="search"
				...
				<div v-else-if="selectedDevice" class="support-device-card">
			...
			<section aria-labelledby="support-classification-heading">
				...
				<div class="support-fields">
					<label>
						...
						<select v-model="form.categoria" :disabled="submitting">
					...
					<label>
						...
						<select v-model="form.prioridad" :disabled="submitting">
			...
			<section aria-labelledby="support-duration-heading">
				...
				<div class="support-time-stack">
					<SupportDurationFields v-model="form.duracion_minutos" :disabled="submitting" />
					<label class="support-date">
						<span>{{ t('empleados', 'Support date') }}</span>
						<input v-model="form.fecha" type="datetime-local" :disabled="submitting">
					</label>
				</div>
			</section>
...
export default {
	...
	data() {
		return {
			...
			searchTimer: null,
			searchSequence: 0,
			selectedDevice: this.initialDevice ? { ...this.initialDevice } : null,
			submitting: false,
			supportSuccess: null,
			form: initialForm(),
		}
	...
	computed: {
		employeeName() {
			return this.selectedDevice?.empleado_displayname
				|| this.selectedDevice?.empleado_uid
				|| t('empleados', 'Unassigned')
		},
		modelName() {
			return [this.selectedDevice?.marca, this.selectedDevice?.modelo].filter(Boolean).join(' ')
		},
		responsibleName() {
			const user = getCurrentUser()
			return user?.displayName || user?.uid || t('empleados', 'Current user')
		},
		formValid() {
			return this.selectedDevice !== null
				...
				&& Number.isInteger(this.form.duracion_minutos)
				&& this.form.duracion_minutos > 0
				&& isValidSupportDate(this.form.fecha)
				&& !this.submitting
		},
		formattedSuccessDuration() {
			return formatSupportDuration(this.supportSuccess?.duracion_minutos)
		},
		supportedDeviceUrl() {
			return this.supportSuccess?.id_equipo
				? `${generateUrl('/apps/empleados/')}#/Inventario?deviceId=${encodeURIComponent(this.supportSuccess.id_equipo)}`
				: ''
		},
	},
	watch: {
	...
	methods: {
		t,
		focusInitialField() {
			if (this.hideDeviceSearch || this.selectedDevice) {
				return
			}
		...
		async searchDevices(query, sequence) {
			...
			this.searchCompleted = false
			try {
				const response = await inventarioService.getEquipos({ search: query, limit: 8, offset: 0 })
		...
		deviceSecondary(device) {
			return [device.nombre_sistema, device.numero_serie, device.empleado_displayname || device.empleado_uid]
				.filter(Boolean)
				.join(' · ')
		},
		selectDevice(device) {
		...
		async submitSupport() {
			...
			const payload = {
				id_equipo: Number(this.selectedDevice.id_equipo),
				categoria: this.form.categoria,
				prioridad: this.form.prioridad,
				detalles: this.form.descripcion.trim(),
			...
			try {
				...
				this.supportSuccess = {
					id_soporte: Number(result?.id_soporte),
					id_reporte: Number(result?.id_reporte),
					id_equipo: Number(result?.id_equipo || payload.id_equipo),
					duracion_minutos: Number(result?.duracion_minutos || payload.duracion_minutos),
...
.support-form-shell *,
.support-form-shell *::before,
.support-form-shell *::after {
	box-sizing: border-box;
}
...
.support-device-main > div {
	display: grid;
	flex: 1;
	min-width: 0;
}

.support-device-main strong,
.support-device-main small {
	overflow: hidden;
	text-overflow: ellipsis;
	white-space: nowrap;
...
.support-device-card dt {
	color: var(--color-text-maxcontrast);
	font-size: 11px;
}

.support-device-card dd {
	margin: 2px 0 0;
	overflow: hidden;
	font-size: 12px;
	font-weight: 700;
...
.support-form-shell :deep(.input-field),
.support-form-shell :deep(.textarea__wrapper),
.support-form-shell :deep(textarea),
.support-form-shell :deep(input) {
	max-width: 100%;
	min-width: 0;
}
...
.support-actions :deep(button) {
	max-width: 100%;
}

.support-success :deep(.notecard__content) {
	display: grid;
	gap: 6px;
}
